...
def start_training():
    train_env = VecNormalize(VecMonitor(SubprocVecEnv([lambda: TaskSpaceEnv(learn_from_dir.absolute()) for _ in range(NUM_CPU)])), gamma=1)
    eval_env = VecNormalize(VecMonitor(DummyVecEnv([lambda: TaskSpaceEnv(learn_from_dir.absolute())])), training=False, norm_reward=False)
    ...
    model = PPO("MlpPolicy", train_env, verbose=1, tensorboard_log=MODEL_DIR, gamma=1, batch_size=256, n_steps=100, learning_rate=1e-4)
