...
from .util import make_environments, process_hyperparameters


def start_training(name, env, reward_func, num_envs, raw_hyperparameters={}, load=False, checkpoint=None):
    ...
    if load:
        ...
        custom_log = {
            ...
            "hyperparameters": raw_hyperparameters
        }
        train_env, eval_env = make_environments(env, reward_func, num_envs)
        processed_hyperparameters = process_hyperparameters(raw_hyperparameters, num_envs)
        model = PPO("MlpPolicy", train_env, verbose=1, tensorboard_log=model_dir, **processed_hyperparameters)
